...
set gdefault
set nojoinspaces
set undofile undodir=~/.local/share/vim/undo//
...
fun! Start()
    ...
    setl buftype=nofile bufhidden=wipe nobuflisted nonu nornu noswapfile nocursorline

    let counter = 0
    call append(0, "enter number:")
    for line in v:oldfiles[0:9]
        let file = fnamemodify(l:line, ":~:.")
        call append('$', '  ['. string(counter) . ']  ' . file)
        exec 'nn <buffer><silent> ' . string(counter) . ' :e ' . v:oldfiles[counter] . '<cr>'
        let counter = counter + 1
    endfor
    nn <buffer> q :q<cr>
    nmap <buffer><expr> <cr> getline('.')[col('.')-1]
